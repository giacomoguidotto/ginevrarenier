      ...
      <AnimatePresence>
        {isEditMode && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            initial={{ opacity: 0 }}
            key="editable-image-chrome"
            transition={{ duration: 0.2 }}
          >
            <button
              className={`absolute inset-0 flex cursor-pointer items-center justify-center transition-all ${uploading ? "bg-background/40 opacity-100" : "bg-background/0 opacity-0 hover:bg-background/40 hover:opacity-100"}`}
                ...
                >
                  <DialogContent>
                    <DialogHeader>
                      ...
                      <DialogDescription>
                        This image will be removed when you save. You can undo
